...
class CardAnimationHelper implements Animator.AnimatorListener,
    ...
    //animation duration
    static final int ANIM_DURATION = 100, ANIM_ADD_REMOVE_DELAY = 50,
            ANIM_ADD_REMOVE_DURATION = 100;
    //animation type
...
//            mCards4JudgeZIndex = new ArrayList<>();
        ...
        for (int i = mCardCount - 1; i >= 0; i--) {
            ...
            CardItem cardItem = new CardItem(child, 0, i,i);
            mCardView.addCardView(cardItem);
//            int y= i>=2 ? 2:i;
            int y=i;
            mZIndexTransformerCommon.transformAnimation(cardItem, mCurrentFraction, mCardWidth, mCardHeight, y, y);
            mTransformerCommon.transformAnimation(child, mCurrentFraction, mCardWidth, mCardHeight, y, y);
            mCards.addFirst(cardItem);
//          mCards4JudgeZIndex.add(cardItem);
//            child.setVisibility(View.INVISIBLE);
            showAnimAdd(child, y * mAnimAddRemoveDelay, y, y == mCardCount - 1);
        }
    }
    ...
    private void showAnimAdd(final View view, int delay, final int position, final boolean isLast) {
        ...
        valueAnimator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
            ...
            public void onAnimationUpdate(ValueAnimator animation) {
                ...
                if (mAnimAddRemoveInterpolator != null) {
                    mTransformerAnimAdd.transformInterpolatedAnimation(view, mAnimAddRemoveInterpolator.getInterpolation(fraction),
                            mCardWidth, mCardHeight, position, position);
                }
            }
        });
...
//    }
    ...
    public LinkedList<CardItem> getmCards(){
        return mCards;
    }
