...
public class SlideUnlockView extends View {
	...
	Handler handler = new Handler() {
		public void handleMessage(android.os.Message msg) {
			if (msg.what == 0) {
				...
				if (x > blockWidth / 2) {
					x = x - blockBackgoundWidth * 3.0f / 100;
					// 刷新界面
					postInvalidate();
					// 设置继续移动
					handler.sendEmptyMessageDelayed(0, 1);
					float progress =  x/ (blockBackgoundWidth -  blockWidth/2);
					onUnLockListener.OnProgress((progress) >=1.0f?1.0f:progress);
				} else {
	...
	protected void onDraw(Canvas canvas) {
		...
		switch (currentState) {
			...
			case STATE_MOVING:
				if (x < blockWidth/2) {
					x = blockWidth/2;
				} else if (x > blockBackgoundWidth - blockWidth/2) {
					x = blockBackgoundWidth - blockWidth/2;
				}
				canvas.drawBitmap(slideUnlockBlock, x - blockWidth/2 , 0, null);
	...
	public void setSlideUnlockBackground(int slideUnlockBackgroundResource) {
		Log.i(TAG, "setSlideUnlockBackground....." + slideUnlockBackgroundResource);
	...
	public boolean isOnBackground(int x, int y) {
		if (x <= slideUnlockBackground.getWidth()
				&& y <= slideUnlockBackground.getHeight()) {
			return true;
		}
		return false;
	}
